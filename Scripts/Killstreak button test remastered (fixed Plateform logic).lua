--Fixed Plateform logic it’s now properly working
...
local function TogglePlatform(state)
    -- Always clean any existing platform(s) first (handles lost refs / re-executes)
    if platformPart then
        pcall(function() platformPart:Destroy() end)
        platformPart = nil
    end
    for _, v in ipairs(workspace:GetChildren()) do
        if v:IsA("Part") and v.Size == Vector3.new(2048,1,2048) and v.Position == Vector3.new(165.5,4345.37,-22.83) then
    ...
    if state then
        ...
        platformPart.Name = "KillstreakPlatform"

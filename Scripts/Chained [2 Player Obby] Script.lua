...
local mineCleanerEnabled = false

local mineDelay = 0.1

local mineLoop
local ragdollLoop
local mineCleanerLoop
...
-- MINE CLEANER
--------------------------------------------------------
local function RemoveMineOrExplosion(obj)
    if obj.Name == "Mine" or obj:IsA("Explosion") then
        pcall(function()
            obj:Destroy()
        end)
    end
...
local mineAddedConnection

local function StartMineCleaner()
    if mineCleanerLoop then return end

    -- Remove existing objects
    for _, obj in ipairs(workspace:GetDescendants()) do
        RemoveMineOrExplosion(obj)
    end

    -- Remove newly-created objects immediately
    mineAddedConnection = workspace.DescendantAdded:Connect(function(obj)
        if mineCleanerEnabled and running then
            RemoveMineOrExplosion(obj)
        end
    end)

    -- Failsafe scan
    mineCleanerLoop = task.spawn(function()
        while mineCleanerEnabled and running do
            for _, obj in ipairs(workspace:GetDescendants()) do
                RemoveMineOrExplosion(obj)
            end
...
local function StopMineCleaner()
    mineCleanerEnabled = false

    if mineAddedConnection then
        pcall(function()
            mineAddedConnection:Disconnect()
        end)

        mineAddedConnection = nil
    end
...
MainTab:CreateToggle({
    Name = "Mine remover (removes any mine only on your side)",
    CurrentValue = false,
    Callback = function(v)
        mineCleanerEnabled = v
